...
class RetinaFace(object):
    ...
    def _letterbox(self, image, size, bg_color):
        """
    ...
    def _priorbox(self, image_size): #image_size Support (320,320) and (640,640)
        anchors = []
    ...
    def get_faces(self, img):
        ...
        letterbox_img, aspect_ratio = self._letterbox(img, (model_height,model_width), 114)  # letterbox缩放
        ...
        outputs = self.rknn.inference(inputs=[i])
        if outputs == None: return None
        loc, conf, landmarks = outputs
        priors = self._priorbox(image_size=(model_height, model_width))
        ...
        landmarks = self._decode_landm(landmarks.squeeze(
            0), priors) 
